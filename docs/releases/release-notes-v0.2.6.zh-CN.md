...
- 本地模型导入使用原生文件选择框，校验 Spine 3.8 和 Atlas 纹理，并通过临时目录完成安装。
  启用失败时保留已导入的文件，并显示实际错误。
- 模型切换、导入完成和试穿恢复统一串行提交，避免保存的配置与正在使用的资源目录不一致。
  连续点击导入不会重复打开选择框。
- 依赖更新保留固定 Bun 版本和冻结锁文件检查。JavaScript 依赖审计、前端与 Rust 严格检查、
  Windows 全新安装和升级数据保留测试均纳入发布门禁。
- Manager 和快捷面板收紧控件间距、统一主题样式，并修复自定义 AI 集成图标空白。
