...
- Windows 原生 watchdog 会低频检查当前会话的 DWM 进程。AMD 驱动导致桌面合成器
  崩溃并重启后，即使 WebGL ticker 仍在运行，也会重建桌宠 WebView，避免透明背景持续变黑。
...
- 219 项 JavaScript 测试和 91 项 Rust 测试通过；项目检查、MCP Bridge 自检、
