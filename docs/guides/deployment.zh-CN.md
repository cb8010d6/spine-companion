...
v0.2.7 将 Avatar Job 存储的锁从按文件年龄判断的标记改为操作系统文件锁。启动升级后的应用
前，请退出所有 v0.2.6 的 Spine Companion 实例；不要让旧版和新版同时使用同一个配置目录。
用户数据目录中可以保留 `avatar-jobs.lock` 文件；拥有锁的进程退出后，操作系统会自动释放该锁。
Avatar Job 读写最多等待该锁 10 秒；如果竞争持续，将返回 busy 错误，不会删除过期标记文件。
