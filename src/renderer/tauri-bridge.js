...
export async function initTauriBridge() {
  ...
  window.companion = {
    ...
    listSessions: () => _tauriInvoke("list_ai_sessions"),
    focusSession: (input) => _tauriInvoke("focus_ai_session", { input }),
    dismissDisplay: (revision) => _tauriInvoke("dismiss_companion_display", { revision }),
    ...
    prepareModelPreview: (sourceId, modelId, acknowledgement = false) => _tauriInvoke("prepare_model_preview", { sourceId, modelId, acknowledgement }),
    onState: (callback) => {
